...
#[derive(Debug)]
pub struct DelayLine<'a, T, const MAX_DELAY: usize> {
    write_ptr: usize,
    delay: usize,
    line: &'a mut [T; MAX_DELAY],
}

impl<'a, T, const MAX_DELAY: usize> DelayLine<'a, T, MAX_DELAY>
where
    T: Copy + Num + Signed + From<f32>,
{
    pub fn new(line: &'a mut [T; MAX_DELAY]) -> Self {
        Self {
            write_ptr: 0,
            delay: 1,
            line,
    ...
    pub fn init(&mut self, buffer: &'a mut [T; MAX_DELAY]) {
        self.line = buffer;
        self.reset();
    }
    ...
    pub fn write(&mut self, sample: T) {
        ...
        self.write_ptr = (self.write_ptr.wrapping_sub(1).wrapping_add(MAX_DELAY)) % MAX_DELAY;
    }
